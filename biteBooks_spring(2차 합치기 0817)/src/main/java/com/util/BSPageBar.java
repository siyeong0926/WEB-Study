...
public class BSPageBar {
	...
	public void setPageBar() {
		// 문자열을 조작할 때 원본이 바뀌지 않는 String클래스 대신에 StringBuilder{싱글스레드안전-속도는 조금 빠름}나
		// StringBuffer{멀티스레드안전-속도는 조금 느림}클래스를 사용하자
		...
		if (totalRecord > 0) {
			...
			for (int i = 0; i < pagePerBlock; i++) {// 전체 페이지 블록 수는 3개이다 1 2 3 >> << 4 5 6 >> << 7
				...
				if (nowBlock * pagePerBlock + i == nowPage) {
					...
					pageLink.append("<a class='page-link'>" + (nowBlock * pagePerBlock + i + 1) + "</a>");// 5번숫자에 대한
																											// 코드이다
				}
